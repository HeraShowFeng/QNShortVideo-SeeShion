...
- (void)viewDidLoad {
    ...
    index ++;
    UIButton *aeEffectButton = [[UIButton alloc] initWithFrame:CGRectMake(0, height + index * (height + 10), self.view.bounds.size.width, height)];
    [aeEffectButton setTitle:@"AE 特效" forState:UIControlStateNormal];
    aeEffectButton.backgroundColor = [UIColor grayColor];
    [aeEffectButton addTarget:self action:@selector(aeEffectButtonEvent:) forControlEvents:UIControlEventTouchDown];
    [self.view addSubview:aeEffectButton];
...
- (void)aeEffectButtonEvent:(id)sender {
    [self.clickFeedback impactOccurred];
    
    SXViewController * aeViewController = [[SXViewController alloc] init];
    [self presentViewController:aeViewController animated:YES completion:nil];
}
