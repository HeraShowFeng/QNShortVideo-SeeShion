...
public class MediaGrid extends SquareFrameLayout implements View.OnClickListener {
    ...
    private SquareText mCheckNumTv;
    private FrameLayout mMediaMaskFl;  //遮罩

    public void setmPicDetail(boolean mPicDetail) {
        this.mPicDetail = mPicDetail;
        mCheckView.setVisibility(mPicDetail ? VISIBLE : GONE);
        mCheckNumTv.setVisibility(mPicDetail ? GONE : VISIBLE);
    }

    //判断图片点击事件是查看大图还是选择图片   true 为查看大图
    private boolean mPicDetail = true;
    ...
    private void init(Context context) {
        ...
        mCheckNumTv = findViewById(R.id.check_num_tv);
        mMediaMaskFl = findViewById(R.id.media_mask_fl);
    ...
    public void onClick(View v) {
        if (mListener != null) {
            if (v == mThumbnail && mPicDetail) {
                mListener.onThumbnailClicked(mThumbnail, mMedia, mPreBindInfo.mViewHolder);
            } else if (v == mCheckView || v == mThumbnail) {
    ...
    public void setCheckedNum(int checkedNum) {
        ...
        mMediaMaskFl.setVisibility(checkedNum > 0 ? VISIBLE : GONE);
        if (checkedNum > 0) {
            mCheckNumTv.setTextNum(checkedNum);
        }
    }
