...
public class AlbumMediaAdapter extends
    ...
    protected void onBindViewHolder(final RecyclerView.ViewHolder holder, Cursor cursor) {
        if (holder instanceof CaptureViewHolder) {
            ...
            //用于判断点击事件是查看大图还是选择图片
            mediaViewHolder.mMediaGrid.setmPicDetail(mSelectionSpec.picDetail);
    ...
    private void notifyCheckStateChanged() {
        ...
        if (mShowSelectFra != null) {
            mShowSelectFra.selectChange(mSelectedCollection.asList());
        }
    ...
    public interface showSelectFra {
        void selectChange(List<Item> uriList);
    }

    public void setShowSelectFra(AlbumMediaAdapter.showSelectFra showSelectFra) {
        this.mShowSelectFra = showSelectFra;
    }

    private showSelectFra mShowSelectFra;

    public List<Item> getSelectData() {
        if (mSelectedCollection == null) {
            return null;
        }
        return mSelectedCollection.asList();
    }
    ...
    public void move(List<Item> data){
        mSelectedCollection.setData(data);
        notifyDataSetChanged();
        if (mCheckStateListener != null) {
            mCheckStateListener.onUpdate();
        }
