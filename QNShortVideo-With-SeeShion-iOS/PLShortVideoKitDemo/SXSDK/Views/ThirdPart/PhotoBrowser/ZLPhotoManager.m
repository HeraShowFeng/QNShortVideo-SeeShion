...
{
    NSString *exportFilePath = [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"%zu.mp4", (size_t)asset]];
 
    ZLVideoExportSession * exportSession = [[ZLVideoExportSession alloc] initWithAsset:asset output:exportFilePath];
    [exportSession setOutputFrameRate:15];
    [exportSession setOutputRange:range];
    
    [exportSession finish:VIDEO_EXPORT_PRESET_HIGH finishHandle:^(BOOL success) {
        if(success)
        {
            dispatch_async(dispatch_get_main_queue(), ^{
        ...
        {
            NSLog(@"Export failed: %@", exportFilePath);
        }
    }];
}
